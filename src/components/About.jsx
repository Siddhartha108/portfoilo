...
function About() {
  ...
  const education = [
    ...
    {
      title: "Completed Master of Computer Applications (MCA)",
      period: "2020-2022",
    },
    {
      title: "Completed Bachelor of Computer Applications (BCA)",
      period: "2016-2019",
    },
  ];
  ...
  return (
    ...
    >
      ...
      <p className="my-5">
        Hello, I&apos;m Siddharth, a passionate Web Developer with a keen eye
      ...
      <p className="leading-relaxed my-5">
        {education.map((item, index) => (
          <span key={index} className=" mb-2">
            [{item.title}{" "}
            <span className="text-red-700 bg-[#cac2f1ef] rounded-sm">
              {item.period}
            </span>
            ]{" "}
          </span>
        ...
        {Object.entries(skills).map(([category, items], index) => (
          <p key={index} className="mb-2 inline">
            ...
            {items.map((skill, skillIndex) => (
              <span
                key={skillIndex}
                className="cursor-pointer hover:text-red-600 hover:scale-105 inline-block mr-1"
              >
                {skill},
              </span>
            ))}
